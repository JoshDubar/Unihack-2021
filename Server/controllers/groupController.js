...
exports.CreateGroup = async function(req,res){
    ...
    try{
    const addToGroup = {"_id" : userId, "name" : req.body.name, isHost : true};
    console.log("working");
    const addGroupToUserList = await User.updateOne({_id : userId}, {$push : {groups : addToGroup}});
...
exports.AddMember = async function(req,res){
    ...
    try {
        ...
        const updatedGroup = await Group.updateOne({ _id : groupId},{$push : {members : newMember}});
        //res.json(updatedGroup);
        const newGroup = {"_id" : memberId, "name" : group.name};
        const addGroupToUserList = await User.updateOne({_id : memberId}, {$push : {groups : newGroup}});
        res.json(updatedGroup);
    }
    catch (err){
        res.json({message : err})
